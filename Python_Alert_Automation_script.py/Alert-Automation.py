...
================================================================================
Alert Rule  : Microsoft 365 Portal Login from Rare Location
================================================================================

VERIFIED FIELD MAP (from live logs-o365.audit-* document):
  user.id                              → full email  e.g. 
  user.name                            → short name  e.g. admin
...
CONFIG = {
    "ES_HOST"              : "URL of elasticsearch host,
    "ELK_API_KEY_PATH"     : "api_key path in same project folder",
    "ES_VERIFY_CERTS"      : # False or True,
    "ES_REQUEST_TIMEOUT"   : 60,
    "ES_MAX_RETRIES"       : 3,

    "ALERT_INDEX"          : ".alerts-security.alerts-default*",
    "TARGET_RULE_NAME"     : "Microsoft 365 Portal Login from Rare Location",
    "ALERT_LOOKBACK_HOURS" : 1,        # ← 1hr buffer for 15-min schedule
    ...
    "RUN_ONCE"             : True,     # ← Task Scheduler triggers externally
...
def discover_rare_location_alerts(
    ...
    try:
        ...
        logger.info("Alert discovery: %d hits", len(hits))
        alerts = []
        for hit in hits:
            src = hit.get("_source", {})
